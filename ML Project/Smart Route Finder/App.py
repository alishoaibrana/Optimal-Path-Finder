...
with col_pickup:
    pickup = st.text_input("Pickup Location", placeholder="e.g., New York City", key="pickup", value="New York City")
with col_dropoff:
    dropoff = st.text_input("Drop-off Location", placeholder="e.g., Times Square", key="dropoff", value="Times Square")
...
if st.button("🚀 Find Route"):
    ...
    drop_lat, drop_lon = get_coordinates(dropoff)
    st.session_state["pickup_coords"] = (pickup_lat, pickup_lon)
    st.session_state["dropoff_coords"] = (drop_lat, drop_lon)

# Load from session or use defaults
pickup_lat, pickup_lon = st.session_state.get("pickup_coords", (40.7128, -74.0060))
drop_lat, drop_lon = st.session_state.get("dropoff_coords", (40.7580, -73.9855))
...
st.code(share_url)
st.button("📋 Copy to Clipboard", on_click=st.write, args=("Copy manually: Ctrl+C",))
...
with tabs[2]:
    ...
    user_note = st.text_area("📝 Add notes (e.g., detours, stops):")
    if st.button("💾 Save Note"):
        st.success("Your note has been saved (session-based).")
